...
def extract_completion_content(response) -> str:
    """Extract content from OpenAI SDK objects, dicts, or string-compatible gateways."""
    ...
    if choices:
        ...
        return getattr(first_choice, "text", None) or ""
...
class AIService:
    ...
    def _log_request(
        ...
        if use_reasoning:
            ...
            print(f"[{prefix}] Temperature: {temperature}", flush=True)
        if max_tokens is not None:
            print(f"[{prefix}] Max Tokens: {max_tokens}", flush=True)
        print(f"[{prefix}] Messages Count: {len(messages)}", flush=True)
        if self._enable_logging:
            print(f"[{prefix}] Messages Detail:", flush=True)
    ...
    def _log_api_error(self, prefix: str, error: Exception, can_retry: bool) -> None:
        print(f"[{prefix}] API 调用失败", flush=True)
        print(f"[{prefix}] 错误类型: {get_error_category(error)}", flush=True)
        print(f"[{prefix}] 错误详情: {str(error)}", flush=True)
        print(f"[{prefix}] 可否降级重试: {can_retry}", flush=True)

    def _log_retry(self, prefix: str) -> None:
        print(f"[{prefix}] 尝试降级重试（移除 reasoning_effort）...", flush=True)
    ...
    def _log_response(
        ...
        if response is not None:
            print(f"[{prefix}] Response Type: {type(response).__name__}", flush=True)
            response_id = getattr(response, "id", None)
            response_model = getattr(response, "model", None)
            response_created = getattr(response, "created", None)
            if response_id:
                print(f"[{prefix}] ID: {response_id}", flush=True)
            if response_model:
                print(f"[{prefix}] Model: {response_model}", flush=True)
            if response_created:
                print(f"[{prefix}] Created: {response_created}", flush=True)
            usage = getattr(response, "usage", None)
            if usage:
                print(f"[{prefix}] Token Usage:", flush=True)
                print(f"  Prompt Tokens: {getattr(usage, 'prompt_tokens', '-')}", flush=True)
                print(f"  Completion Tokens: {getattr(usage, 'completion_tokens', '-')}", flush=True)
                print(f"  Total Tokens: {getattr(usage, 'total_tokens', '-')}", flush=True)
        if raw_length is not None:
            print(f"[{prefix}] Raw Content Length: {raw_length}", flush=True)
        if filtered_length is not None:
            print(f"[{prefix}] Filtered Content Length: {filtered_length}", flush=True)
        if self._enable_logging and content is not None:
            print(f"[{prefix}] Content:", flush=True)
            print(content, flush=True)
        print("=" * 80 + "\n", flush=True)

    def _log_exception(self, prefix: str, error: Exception) -> None:
        print(f"[{prefix}] Exception: {str(error)}", flush=True)
        print(f"[{prefix}] Exception Type: {type(error).__name__}", flush=True)
        if self._enable_logging:
            import traceback
    ...
    async def stream_complete(
        ...
        try:
            ...
            self._log_response(
                ...
                content=full_response if self._enable_logging else None,
            )
    ...
    async def complete(
        ...
        try:
            ...
            self._log_response(
                ...
                content=filtered_content if self._enable_logging else None,
            )
